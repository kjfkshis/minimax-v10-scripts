        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                ...
                if (currentSessionId !== null) {
                    ...
                    // Chunks cũ không có sessionId sẽ bị loại bỏ
                    chunks = chunks.filter(chunk => chunk.sessionId === currentSessionId);
                    addLogEntry(`🔍 Đã lọc chunks theo session ID: ${currentSessionId} (${chunks.length} chunks)`, 'info');
                } else {
                    // Nếu không có session ID, có thể là lỗi hoặc chưa khởi tạo session
                    // Để an toàn, chỉ lấy chunks không có sessionId (chunks cũ) hoặc trả về rỗng
                    addLogEntry('⚠️ Cảnh báo: Không có session ID khi đọc chunks. Có thể đọc nhầm chunks cũ.', 'warning');
                    chunks = chunks.filter(chunk => !chunk.hasOwnProperty('sessionId') || chunk.sessionId === null);
                }
...
async function waitForVoiceModelReady() {
    ...
    if (startBtn) {
        startBtn.addEventListener('click', async () => {
            ...
            // TẠO SESSION ID MỚI cho lần tạo audio này
            currentSessionId = Date.now(); // Sử dụng timestamp làm session ID duy nhất
            addLogEntry(`🆔 Đã tạo session ID mới: ${currentSessionId}`, 'info');

            // XÓA CHUNKS CŨ TỪ INDEXEDDB (NÂNG CẤP MỚI) - Đảm bảo hoàn tất trước khi bắt đầu
            try {
                await clearChunkDB();
                addLogEntry('✅ Đã xóa tất cả chunks cũ từ IndexedDB', 'success');
            } catch (err) {
